...
class LiverpoolFreePress:
    ...
    def download_newspapers(self):
        ...
        sites = [f"https://freepressarchive.com/issues/{site['href']}" for site in soup.find_all('a', href=True) if
                 'issue' in site['href']]
        for site in sites:
            ...
            for pdf in pdfs:
                ...
                if response.status_code == 200:
                    ...
                    with open('download_results.txt', 'a') as f:
                        f.write(
                            f"{paper_number}/{filename} was not downloaded,it had response status code {response.status_code}\n")
                    print(
                        f"{paper_number}/{filename} was not downloaded,it had response status code {response.status_code}")

    # The following method will download the missing newspapers
