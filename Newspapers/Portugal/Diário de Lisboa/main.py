...
class DiariodeLisboa:
    ...
    # The following method will check if a date was downloaded or not.If it was not downloaded, this method will download it
    def check_date(self,date:datetime.datetime):
        if self.start_date <= date <= self.end_date:
            ...
            if date.day in days:
                ...
                images = [img.replace("d3", "d2").replace("D3", "D2").replace("jpg", "png") for img in images]
                for i in range(len(images)):
                    self.check_image(date, images[i], i + 1)
            else:
                with open("download_results.txt", "a") as f:
                    f.write(f"{date.day}-{date.month}-{date.year} is not on the archive\n")
                print(f"{date.day}-{date.month}-{date.year} is not on the archive")
    ...
    # The following method will check the images to see if they were downloaded or not
    def check_image(self,date:datetime.datetime,image_link:str,count:int):
        formatted_date = f"{date.day}-{date.month}-{date.year}"
        try:
            os.mkdir(f"{date.year}/{formatted_date}")
        except FileExistsError:
        ...
        if f"{count}.png" not in os.listdir(f"{date.year}/{formatted_date}"):
            response = requests.get(url=image_link)
            if response.status_code == 200:
                with open(f"{date.year}/{formatted_date}/{count}.png", "wb") as f:
                    f.write(response.content)
                ...
                if new_response.status_code == 200:
                    ...
                    with open("download_results.txt", "a") as f:
                        f.write(
                            f"{date.year}/{formatted_date}/{count}.png was not downloaded,it had response status code {new_response.status_code}\n")
                    print(
                        f"{date.year}/{formatted_date}/{count}.png was not downloaded,it had response status code {new_response.status_code}")
    ...
    def check_all(self):
        self.check_d1_d2(self.start_date,self.end_date)
