...
class Evrensel:
    ...
    # The following method will check if a particular number got downloaded or not
    def check(self,number:int):
        if self.first <= number <= self.last:
            if f"{number}.jpg" not in os.listdir():
                website = f"https://www.evrensel.net/fotograf/5/birinci-sayfa/s/{number}"
                self.google_driver.get(url=website)
                soup = BeautifulSoup(self.google_driver.page_source, "lxml")
                for image in soup.find_all("img", src=True, alt=True):
                    if "https://www.evrensel.net/upload/fotograf/" in image[
                        "src"] or "https://www.evrensel.net/files/gallery/big/" in image["src"]:
                        source = image['src']
    ...
    def check_all(self):
        self.check_n1_n2(self.first,self.last)
