...
class LegoInstructions:
    ...
    def get_years(self):
        ...
        select_tags = [select.find_all("option") for select in lego_soup.find_all("select",class_="c-form-item__control") if select["id"] == "filter-year"][0]
    ...
    def get_themes(self):
        ...
        select_tags = [select.find_all("option") for select in lego_soup.find_all("select", class_="c-form-item__control") if select["id"] == "filter-theme"][0]
    ...
    def download_theme(self,theme:str):
        if theme in self.theme_clicks:
            ...
            lego_sets = sorted(list(set([f"https://www.lego.com{anchor_tag['href']}" for anchor_tag in year_soup.find_all("a",href=True) if "View Instructions" in anchor_tag.text])))
            os.makedirs(theme)
